...
* Sci-Hub is effectively unavailable as a download source, and the `--db scihub` path is expected to fail. The mirrors still answer, but none of them return a PDF: some serve a placeholder page carrying only an advertisement, others serve a robot-check interstitial, and the `sci-net.xyz`/`sci-net.ru` hosts serve an account landing page instead of the article. These are server-side gates, so no change to the mirror list fixes them — use Unpaywall for open access and Anna's Archive for the rest. Note also that mirror domains are frequently abandoned and re-registered by unrelated parties, so a domain that resolves is not necessarily still the service it names.
