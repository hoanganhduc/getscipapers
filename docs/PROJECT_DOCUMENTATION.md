...
- **Anna’s Archive (`getpapers.py`, `download_from_anna_archive`)**: Included in the multi-source download path for fallback retrieval, resolving a DOI through `/scidb/` and then following the md5 link to the file. The `annas-archive.gl`, `.pk` and `.gd` hosts sit behind DDoS-Guard, which answers every content path with HTTP 403 from datacenter addresses while leaving `/` reachable, so the route is only usable from an egress the challenge accepts. A 403 is in `PROXY_RETRY_STATUSES`, so a configured proxy is retried automatically. Only those three hosts are real mirrors: other resolving `annas-archive.*` domains are parked lookalikes that answer 200 with an affiliate bounce page, which is why the fetched bytes are checked for PDF magic bytes before anything is written.
